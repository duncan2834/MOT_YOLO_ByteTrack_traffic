...
from bytetrack.utils import box_iou_batch
...
class ObjectTracking:
    def __init__(self, input_video_path, output_video_path):
        ...
        self.LINE_START = Point(50, 600)
        self.LINE_END = Point(1230, 600)
        ...
        self.line_zone = LineZone(start=self.LINE_START, end=self.LINE_END) # vạch đếm phương tiện
        # create instance of BoxAnnotator
        self.box_annotator = BoxAnnotator(thickness=2, text_thickness=2, text_scale=2)
        # create instance of TraceAnnotator
        self.trace_annotator = TraceAnnotator(thickness=4, trace_length=50)
        # create LineZoneAnnotator instance
        self.line_zone_annotator = LineZoneAnnotator(thickness=2, text_thickness=2, text_scale=2)
    ...
    def callback(self, frame, index):
        ...
        # Phát hiện các vật thể, lọc ra các class quan tâm
        ...
        # update line counter
        self.line_zone.trigger(detections) # Cập nhật số lượng object cắt qua đường line (đếm object)
        # return frame with box and line annotated result
        return  self.line_zone_annotator.annotate(annotated_frame, line_counter=self.line_zone)
    
    def process(self):
        with VideoSink(target_path=self.output_video_path, video_info=self.video_info) as sink: # Duyệt qua từng frame của video đầu vào.
