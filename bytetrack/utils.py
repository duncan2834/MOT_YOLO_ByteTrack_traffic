...
def _mask_iou_batch_split(masks_true, masks_detection):
    """
    Internal function.
...
def mask_iou_batch(masks_true, masks_detection, memory_limit= 1024 * 5):
    """
    Compute Intersection over Union (IoU) of two sets of masks -
        `masks_true` and `masks_detection`.
...
def resize_masks(masks, max_dimension = 640):
    """
...
def clip_boxes(xyxy, resolution_wh):
    """
...
def approximate_polygon(
    polygon, percentage, epsilon_step = 0.05
):
    """
...
def extract_ultralytics_masks(yolov8_results):
...
def calculate_masks_centroids(masks):
    """
...
def validate_class_id(class_id, n):
    expected_shape = f"({n},)"
...
def validate_confidence(confidence, n) :
    expected_shape = f"({n},)"
...
def validate_tracker_id(tracker_id, n):
    expected_shape = f"({n},)"
...
def is_data_equal(data_a, data_b):
    """
